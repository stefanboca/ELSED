 ...
 * - Endpoints
 *
 * The implementation is highly optimized to add or subtract new pixels and
 * recompute all parameters. Internally,
 */
class FullSegmentInfo {
public:
  // Creates the FullSegmentInfo object but do not initialize internal
  // structures
  ...
  // Checks if the point (x, y) is an inlier given the current line equation and
  // a distance th.
  inline bool isInlier(int x, int y, double lineFitErrThreshold) {
    const double pointToLineDis =
        equation[0] * x + equation[1] * y + equation[2];
  ...
  // isPixelAtTheEnd True if the pixel is at the end of the segment or false if
  // it is at the start
  void addPixel(int x, int y, int pixelIndexInEdge,
  ...
  inline double getFitError() const {
    ...
    for (int i = firstPxIndex; i <= lastPxIndex; i++) {
      dist = equation[0] * (*pixels)[i].x + equation[1] * (*pixels)[i].y +
             equation[2];
      fitError += dist * dist;
    }
    return fitError / N;
  ...
  // True if the segment has been already extended in the direction of the first
  // endpoint
...
private:
  ...
  // Computes the segment endpoints given the internal parameters (very
  // efficient)
  ...
  // we invert the least squares and use y as independent coordinate and x as
  // dependent coord.
  bool isHorizontal;
  ...
  // Indicates whenever the list of pixels from firstPxIndex to lastPxIndex is
  // sorted
